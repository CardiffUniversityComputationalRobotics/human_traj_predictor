...
class MovePlanPredInterfaceMultiEnv(object):
    ...
    def forward(
        ...
        for i in range(self.nenv):
            ...
            # where 5 includes [x, y, v_x, v_y, timestep]
            print("=====================")
            # print(ped_mask_all.shape)
            print(ped_pos_all[i, 0, :])
            print(ped_mask[i])
            print("++++++++++++++++++++")

            ob_ped_pos, ob_ped_mask, col_ind_pres_peds = self.filter_curr_ob(
                ped_pos, ped_mask
            )
            ob_veh_pos, ob_veh_mask, col_ind_pres_vehs = self.filter_curr_ob(
                veh_pos, veh_mask
            )

            pred_pos = torch.zeros(
                (self.config.sim.pred_len, self.config.sim.max_human_num, 5)
            )
            ...
            pred_cov = (
                ...
                .unsqueeze(0)
                .unsqueeze(0)
                .repeat(self.config.sim.pred_len, self.config.sim.max_human_num, 1, 1)
            )
            ...
            prediction_mask = self.mask_creation(
                ped_pos, ped_mask
            )  # shape: (pred_seq_len, num_peds)
        ...
        output_traj_cov = output_traj_cov.permute(
            0, 2, 1, 3, 4
        )  # (n_env, num_peds, obs_seq_len, 2, 2)
    ...
    def filter_curr_ob(self, pos, mask):
        ...
        columns_to_keep = (
            num_avail_fram != 0
        ).nonzero()  # of shape (num_valid_columns, 1)
