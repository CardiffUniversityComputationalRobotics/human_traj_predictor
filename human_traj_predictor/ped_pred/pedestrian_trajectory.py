...
from human_traj_predictor.ped_pred.Interaction import (
    getInteractionGridMask,
    getSequenceInteractionGridMask,
)
from human_traj_predictor.ped_pred.helper import *
from launch_ros.substitutions import FindPackageShare


human_traj_predictor_dir = FindPackageShare(package="human_traj_predictor").find(
...
class traj_prediction:
    def __init__(self):
        ...
        prefix = (
            human_traj_predictor_dir + "/human_traj_predictor/ped_pred/TrainedModel/"
        )
        ...
        self.saved_args.use_cuda = True

        save_tar_name = method_name + "_lstm_model"

        self.net = get_model(4, self.saved_args, True)
    ...
    def forward(
        self, x_seq, mask, x_seq_veh, mask_veh, ego_obs, ego_pre_planned_traj, timestamp
    ):
        ...
        if self.saved_args.method == 4:  # collision grid
            grid_seq, grid_TTC_seq = getSequenceInteractionGridMask(
                x_seq,
                mask,
                x_seq,
                mask,
            ...
            grid_seq_veh_in_ped, grid_TTC_veh_seq = getSequenceInteractionGridMask(
                x_seq,
                mask,
                x_seq_veh,
                mask_veh,
                self.saved_args.TTC_veh,
                self.saved_args.D_min_veh,
                self.saved_args.num_sector,
                self.saved_args.use_cuda,
                is_heterogeneous=True,
                sequence_ego=ego_obs,
            )
        ...
        if self.saved_args.method == 3:  # vanilla lstm
            ...
            ret_x_seq, dist_param_seq = self.sample(
                obs_traj,
                obs_mask,
                self.net,
                self.saved_args,
                ...
                None,
                timestamp,
                ego_pre_planned_traj,
            )
        elif self.saved_args.method == 4:  # collision grid
            # Extract the observed part of the trajectories
            obs_traj, obs_mask, obs_grid, obs_grid_TTC = (
                x_seq.clone(),
                mask.clone(),
                grid_seq.copy(),
                grid_TTC_seq.copy(),
            )
            obs_grid_veh_in_ped, obs_grid_TTC_veh = (
                grid_seq_veh_in_ped[: self.obs_length],
                grid_TTC_veh_seq[: self.obs_length],
            )
        ...
        ret_x_seq = revert_postion_change(
            ret_x_seq.cpu(),
            extended_mask,
            first_values_dict,
            orig_x_seq,
        ...
        dist_param_seq[:, :, 0:2] = revert_postion_change(
            dist_param_seq[:, :, 0:2].cpu(),
            extended_mask,
            first_values_dict,
            orig_x_seq,
            self.obs_length,
        ...
        return (
            ret_x_seq[self.obs_length :, :, :5],
            dist_param_seq[self.obs_length :, :],
        )
    ...
    def sample(
        ...
        ret_x_seq = Variable(
            torch.zeros(self.obs_length + self.pred_length, numx_seq, x_seq.shape[2])
        )
        dist_param_seq = Variable(
            torch.zeros(self.obs_length + self.pred_length, numx_seq, 5)
        )
        ...
        for tstep in range(self.obs_length - 1):
            ...
            out_obs, hidden_states, cell_states = net(
                x_seq[tstep, :, :].view(1, numx_seq, x_seq.shape[2]),
                [grid_t],
                hidden_states,
                cell_states,
        ...
        ret_x_seq[tstep + 1, :, 5:9] = x_seq[
            -1, :, 5:9
        ]  # covariances of the trjecotries generated by the Kalman filter
        last_observed_frame_prediction = ret_x_seq[tstep + 1, :, :2].clone()
        ret_x_seq[tstep + 1, :, :2] = x_seq[
            -1, :, :2
        ]  # storing the last GT observed frame here to ensure this is used in the next for loop and then
        ...
        for tstep in range(self.obs_length - 1, self.pred_length + self.obs_length - 1):
            ...
            outputs, hidden_states, cell_states = net(
                ret_x_seq[tstep].view(1, numx_seq, ret_x_seq.shape[2]),
                [prev_grid],
                hidden_states,
                cell_states,
                last_obs_frame_mask.view(1, numx_seq),
                x_seq_veh[tstep, :, :].view(1, numx_seq_veh, x_seq_veh.shape[2]),
                [prev_grid_veh_in_ped],
                mask_veh[tstep, :].view(1, numx_seq_veh),
                [prev_TTC_grid],
                [prev_TTC_grid_veh],
            )
            ...
            ret_x_seq_convert = revert_postion_change(
                ret_x_seq_convert.cpu(),
                extended_mask,
                first_values_dict,
                orig_x_seq,
                saved_args.obs_length,
                infer=True,
            )

            ret_x_seq_convert[tstep + 1, :, 2] = (
                ret_x_seq_convert[tstep + 1, :, 0] - ret_x_seq_convert[tstep, :, 0]
            ) / timestamp  # vx
            ret_x_seq_convert[tstep + 1, :, 3] = (
                ret_x_seq_convert[tstep + 1, :, 1] - ret_x_seq_convert[tstep, :, 1]
            ) / timestamp  # vy
            ...
            converted_pedlist = [
                i for i in range(numx_seq) if last_obs_frame_mask[i] == 1
            ]
            list_of_x_seq = Variable(torch.LongTensor(converted_pedlist))

            # Get their predicted positions
            current_x_seq = torch.index_select(
                ret_x_seq_convert[tstep + 1], 0, list_of_x_seq
            )

            converted_vehlist = [
                i for i in range(numx_seq_veh) if mask_veh[tstep + 1, i] == 1
            ]
            ...
            prev_grid, prev_TTC_grid = getInteractionGridMask(
                ...
                saved_args.TTC,
                saved_args.D_min,
                saved_args.num_sector,
            )
            prev_grid_veh_in_ped, prev_TTC_grid_veh = getInteractionGridMask(
                current_x_seq.data.cpu(),
                current_x_seq_veh.data.cpu(),
                saved_args.TTC_veh,
                saved_args.D_min_veh,
                saved_args.num_sector,
                is_heterogeneous=True,
            ...
            prev_grid_veh_in_ped = Variable(
                torch.from_numpy(prev_grid_veh_in_ped).float()
            )
