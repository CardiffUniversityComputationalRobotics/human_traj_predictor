...
def Time2Conflict(
    current_position, current_velocity, other_position, other_velocity, Social_dis
):
    """
...
def approach_angle_sector(current_velocity, other_velocity, num_sector):
    ...
    det = (
        current_velocity[0] * other_velocity[1]
        - other_velocity[0] * current_velocity[1]
    )
    angle = math.atan2(det, dot) * (180 / math.pi)  # the value is between -180 and 180
    ...
    if (
        angle == 360
    ):  # an edge case. To ensure that we do not get out of bound in matrix dimension later on
        approach_cell = num_sector - 1

    return approach_cell


def getInteractionGridMask(
    frame,
    frame_other,
    TTC_min,
    d_min,
    ...
    for real_frame_index, other_real_frame_index in itertools.product(
        list_indices, list_indices_other
    ):
        ...
        t_conflict = Time2Conflict(
            current_position, current_velocity, other_position, other_velocity, d_min
        )
        ...
        approach_cell = approach_angle_sector(
            current_velocity, other_velocity, num_sector
        )
        ...
        if is_occupancy:
            ...
            frame_mask[
                real_frame_index,
                other_real_frame_index,
                time_cell * num_sector + approach_cell,
            ] = 1
            ...
            TTC_mask[
                ...
                TTC_min[0] - t_conflict
            )

    return frame_mask, TTC_mask


def getSequenceInteractionGridMask(
    sequence,
    avail_mask,
    sequence_veh,
    avail_mask_veh,
