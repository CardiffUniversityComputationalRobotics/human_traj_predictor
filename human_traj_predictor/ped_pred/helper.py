...
def position_change(x_seq, avail_mask):
    ...
    first_frame_apearing = torch.argmax(
        avail_mask.to(torch.int), dim=0
    )  # shape: (num_ped)
    ...
    first_values_dict = dict(
        zip(
            existing_ped_indexes.tolist(),
            x_seq[
                first_frame_apearing[existing_ped_indexes].tolist(),
                existing_ped_indexes.tolist(),
                :2,
            ],
        )
    ...
    vectorized_x_seq[
        ...
        existing_ped_indexes.tolist(),
        :2,
    ] = torch.zeros(len(existing_ped_indexes), 2)
    ...
    for ind, frame in enumerate(x_seq):
        ...
        for ped_indx in range(len(avail)):
            if avail[ped_indx] == 1:
                ...
                if (
                    first_presence_flag[ped_indx] == 0
                ):  # this frame is the first frame where this pedestrian apears
                    vectorized_x_seq[ind, ped_indx, 0:2] = (
                        frame[ped_indx, 0:2] - first_values_dict[ped_indx][0:2]
                    )  # this should always give (0,0)
                    latest_pos[ped_indx] = frame[ped_indx, 0:2]
                    first_presence_flag[ped_indx] = 1
                else:
                    vectorized_x_seq[ind, ped_indx, 0:2] = (
                        frame[ped_indx, 0:2] - latest_pos[ped_indx]
                    )
                    latest_pos[ped_indx] = frame[ped_indx, 0:2]
...
def sample_gaussian_2d(mux, muy, sx, sy, corr, mask):
    ...
    return (
        next_x,
        next_y,
    )  # The value of next_x and next_y for those agent not present in this frame is zero


def revert_postion_change(
    x_seq,
    avail_mask,
    first_values_dict,
    orig_x_seq,
    ...
    first_values_list = [
        first_values_dict[key] for key in sorted(first_values_dict.keys())
    ]
    first_values = torch.stack(first_values_list)  # shape: (num_ped, 2)

    # observed part:
    absolute_x_seq[1:obs_length, :, :2] = (
        x_seq[1:obs_length, :, :2] + orig_x_seq[: obs_length - 1, :, :2]
    )
    ...
    if len(x_seq) > obs_length:  # if we have prediction part
        if infer == True:
            ...
            absolute_x_seq[obs_length:, :, :2] = torch.cumsum(
                absolute_x_seq[obs_length:, :, :2], dim=0
            )
        else:
            absolute_x_seq[obs_length:, :, :2] = (
                absolute_x_seq[obs_length:, :, :2]
                + orig_x_seq[obs_length - 1 : -1, :, :2]
            )
...
def revert_postion_change_seq2(
    x_seq,
    avail_mask,
    first_values_dict,
    orig_x_seq,
        ...
        for ped_indx in range(len(avail)):
            if avail[ped_indx] == 1:
                if (
                    ...
                    absolute_x_seq[ind, ped_indx, 0:2] = (
                        frame[ped_indx, 0:2] + first_values_dict[ped_indx][0:2]
                    )
                    latest_pos[ped_indx] = absolute_x_seq[
                        ind, ped_indx, 0:2
                    ...
                    if (
                        infer == True and ind >= obs_length
                    ):  # we have to rely on the algorithm's own prediction for the next state
...
def KF_covariance_generator(
    x_seq,
    mask,
    dt,
    plot_bivariate_gaussian3=None,
    ax2=None,
    x_orig=None,
    Pedlist=None,
    lookup=None,
    use_cuda=None,
    first_values_dict=None,
    obs_length=None,
):
    """
    ...
    )  # only passing the position covariance matrix
...
def cov_mat_generation(
    dist_param,
):  # the input should be dist_param after the scaling has been done in the gen_Coef
    """
