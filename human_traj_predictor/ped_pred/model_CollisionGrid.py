...
class CollisionGridModel(nn.Module):
    def __init__(self, args, infer=False):
        ...
        self.tensor_embedding_layer = nn.Linear(
            self.num_TTC * self.num_sector, self.embedding_size
        )  # uncomment this (new - ped)
        ...
        self.tensor_embedding_layer_veh = nn.Linear(
            self.num_TTC_veh * self.num_sector, self.embedding_size
        )
    ...
    def getSocialTensor(self, grid, hidden_states, veh_tensor=False):
        ...
        if veh_tensor == True:
            social_tensor = Variable(
                torch.zeros(
                    numNodes, self.num_TTC * self.num_sector, self.embedding_size
                )
            )
        else:  # for ped in ped grid.
            social_tensor = Variable(
                torch.zeros(numNodes, self.num_TTC * self.num_sector, self.rnn_size)
            )
        if self.use_cuda:
        ...
        if veh_tensor == True:
            ...
            social_tensor = social_tensor.view(
                numNodes, self.num_TTC * self.num_sector * self.rnn_size
            )
    ...
    def getSocialTensor2(self, grid, hidden_states, grid_TTC):
        ...
        # Construct the variable
        social_tensor = Variable(
            torch.zeros(numNodes, self.num_TTC * self.num_sector, third_dim)
        )
        if self.use_cuda:
            social_tensor = social_tensor.cuda()
        ...
        # Reshape the social tensor
        social_tensor = social_tensor.view(
            numNodes, self.num_TTC * self.num_sector * third_dim
        )
    ...
    def OnePerGridCell(self, grid_current, grid_TTC_current):
        ...
        for col in valid_col_index:
            ...
            ttc = grid_TTC_current[
                rows, col_ind
            ]  # a smaller tensor or only ttc of conflicting agents presnet in that cell
    ...
    def forward(self, *args):
        ...
        outputs = Variable(
            torch.zeros((self.seq_length - 1) * numNodes, self.output_size)
        )
        if self.use_cuda:
        ...
        for framenum, frame in enumerate(input_data):
            ...
            nodes_current = frame[
                list_of_nodes, :2
            ]  # Getting only the x and y of each pedestrian for the input.
            ...
            social_tensor = grid_TTC_current.max(1)[
                0
            ]  # this should become of size: num_agent * num_sector
            ...
            if (
                grid_TTC_current_veh.shape[1] == 0
            ):  # meaning we do not have a vehicle in this sequence
                social_tensor_veh = torch.zeros(
                    (grid_TTC_current_veh.shape[0], grid_TTC_current_veh.shape[2])
                )
                if self.use_cuda:
                    social_tensor_veh = social_tensor_veh.cuda()
            else:
                social_tensor_veh = grid_TTC_current_veh.max(1)[
                    0
                ]  # same as for pedestrians

            tensor_embedded_veh = self.dropout(
                self.relu(self.tensor_embedding_layer_veh(social_tensor_veh))
            )

            # Concat input
            concat_embedded = torch.cat(
                (input_embedded, tensor_embedded, tensor_embedded_veh), 1
            )
