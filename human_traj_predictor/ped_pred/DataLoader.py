...
class DataLoader:
    ...
    def __init__(self, phase="train"):
        ...
        fps = (
            2 / self.downsample_step
        )  # (CITR: 29.97, DUT:23.98, ETH/UCY: 2.5, HBS: 2, SDD: 30)
        self.timestamp = 1 / fps
        self.phase = phase

        base_dataset = [
            human_traj_predictor_dir + "/human_traj_predictor/ped_pred/Data/HBS/hbs.csv"
        ]
        ...
        self.data_file_train = os.path.join(
            human_traj_predictor_dir
            + "/human_traj_predictor/ped_pred/Data/preprocessed/",
            "trajectories_train.cpkl",
        )
        self.data_file_test = os.path.join(
            human_traj_predictor_dir
            + "/human_traj_predictor/ped_pred/Data/preprocessed/",
            "trajectories_test.cpkl",
        )
        self.data_file_vali = os.path.join(
            human_traj_predictor_dir
            + "/human_traj_predictor/ped_pred/Data/preprocessed/",
        ...
        if self.phase == "train":
            ...
            raise Exception(
                'the phase input for dataloder should be either "train", "test" or "val"'
            )

        if not (
            os.path.exists(self.data_file)
        ):  # all the files will be created at the first run.
    ...
    def frame_preprocess(self, data_dirs, data_file, phase):
        ...
        for directory in data_dirs:
            ...
            considered_frames = np.arange(
                min_frame, max_frame, self.downsample_step
            ).tolist()
            df = df_orig.loc[df_orig["frame_id"].isin(considered_frames)]

            df_ped_orig = df.loc[
                (df["label"].isin(["pedestrian", "ped"]))
            ]  # !! Check these lables for each new dataset you want to add later
            df_veh_orig = df.loc[
                (df["label"].isin(["car", "cart", "veh", "vehicle"]))
            ]  # !!! Check these lables for each new dataset you want to add later
            self.target_ped_ids = np.array(
                df_ped_orig.drop_duplicates(
                    subset={"agent_id"}, keep="first", inplace=False
                )["agent_id"]
            )
            self.target_veh_ids = np.array(
                df_veh_orig.drop_duplicates(
                    subset={"agent_id"}, keep="first", inplace=False
                )["agent_id"]
            )
            ...
            data = np.swapaxes(
                data, 0, 1
            )  # Now the frame id will propogate through the columns and the frames are all in the first row
            ...
            for veh_id in self.target_veh_ids:
                ...
                for ind, frame in enumerate(frameList):
                    ...
                    # Extract peds list
                    pedsList = pedsInFrame[
                        1, :
                    ].tolist()  # Grabing the agent_id of all the pedestrians in this specific frame
        ...
        pickle.dump(
            (
                all_frame_data_ego_phase,
                egoList_data_phase,
                all_frame_data_ped_phase,
                frameList_data_phase,
                numPeds_data_phase,
                pedsList_data_phase,
                target_ids_phase,
                orig_data_phase,
                all_frame_data_veh_phase,
                numVehs_data_phase,
                vehsList_data_phase,
                target_ids_veh_phase,
                orig_data_veh_phase,
                scenario_num_list_phase,
            ),
            f,
            protocol=2,
        )
    ...
    def load_preprocessed(self, data_file):
        ...
        self.egoData = raw_data[
            0
        ]  # all_frame_data_ego  from the frame_preprocess function
        self.egoList = raw_data[1]
        self.data_ped = raw_data[
            2
        ]  # all_frame_data  from the frame_preprocess function
        self.frameList = raw_data[
            3
        ]  # frameList_data  from the frame_preprocess function
        self.numPedsList = raw_data[
            4
        ]  # numPeds_data  from the frame_preprocess function
        ...
        self.num_features = (
            self.egoData[0][0].shape[1] - 1
        )  #  -1 to not consider agent id in the count
    ...
    def extend_traj(self, pos_veh, mask_veh, ext_len):
        ...
        for t in range(ext_len):
            ...
            ext_pos[t, last_pres_vehs_ind, 4] = self.timestamp * np.ones(
                len(last_pres_vehs_ind)
            )
            last_pos = next_pos
    ...
    def extend_traj2(self, pos_veh, VehsList, lookup_veh, ext_len):
        ...
        timestep_rp = (
            timestep.unsqueeze(dim=1).unsqueeze(2).repeat(1, len(last_pres_vehs_ID), 2)
        )
        ...
        ext_pos[:, last_pres_vehs_ind, 4] = self.timestamp * torch.ones(
            ext_len, len(last_pres_vehs_ID)
        )
